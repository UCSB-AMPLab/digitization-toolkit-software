...
`PUBLIC_API_BASE` is read at runtime by the SvelteKit server (via `$env/dynamic/public`), not compiled into the frontend image — so a plain restart with the updated `.env` picks it up; no image rebuild is required for this variable alone. If other build-time inputs changed, rebuild as usual:
